...
// ---- Admin: list removed with current hide flags
app.get('/api/admin/removed', requireAdmin, async (req,res)=>{
  const ref = req.query.date || new Date().toISOString().slice(0,10);

  // reuse discipline computation
  const names = await getAllNames();
  ...
  const sql = `
    ...
    SELECT a.name, a.w1, a.w2, COALESCE(f.hide_removed,false) AS hidden
    FROM agg a
    LEFT JOIN admin_flags f ON f.name = a.name
    ORDER BY a.name;
  `;
  const { rows } = await pool.query(sql, [ref, names]);

  // removed = w1+w2 < 60 (per your discipline rule)
  const items = rows
    .filter(r => (r.w1 + r.w2) < 60)
    .map(r => ({ name:r.name, minutes:(r.w1+r.w2), hidden:r.hidden }));
  res.json({ items });
});

// ---- Admin: toggle hide
app.post('/api/admin/hide', requireAdmin, async (req,res)=>{
  const { name, hide } = req.body || {};
  if(!name) return res.status(400).json({ error:'name required' });
  await pool.query(`
    INSERT INTO admin_flags(name, hide_removed) VALUES($1,$2)
    ON CONFLICT (name) DO UPDATE SET hide_removed = EXCLUDED.hide_removed
  `, [name, !!hide]);
  res.json({ ok:true });
});
